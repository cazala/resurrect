...
use resurrect_core::{
    ...
    MAX_TTL_SECONDS, NetworkDescriptor, PeerCandidate, RESURRECT_VERSION,
};
use thiserror::Error;
...
impl Default for ScannerConfig {
    fn default() -> Self {
        Self {
            ...
            max_candidates: resurrect_core::DEFAULT_MAX_ACTIVE_CANDIDATES,
            max_endpoints_per_record: resurrect_core::DEFAULT_MAX_ENDPOINTS_PER_RECORD,
...
/// Recent-window Resurrect registry scanner.
#[derive(Debug)]
                ...
                .take(self.config.max_logs_per_scan - logs_processed)
            {
                ...
                let policy = AnnouncementPolicy {
                    ...
                    max_record_bytes: usize::try_from(constants.max_record_bytes)
                        .unwrap_or(resurrect_core::DEFAULT_MAX_RECORD_BYTES),
...
fn verify_constants(
    ...
    if constants.version != RESURRECT_VERSION
        ...
        || usize::try_from(constants.max_record_bytes).ok()
            != Some(resurrect_core::DEFAULT_MAX_RECORD_BYTES)
    {
...
pub enum ScanError {
    ...
    #[error("deployed registry constants are not Resurrect v1: {0:?}")]
