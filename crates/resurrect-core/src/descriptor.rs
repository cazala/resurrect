...
/// Complete application configuration required to discover Resurrect peers.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct NetworkDescriptor {
    /// Resurrect protocol version. Must be `1`.
    pub resurrect_version: u32,
...
impl NetworkDescriptor {
    ...
    /// Validates all cross-field Resurrect v1 invariants.
    ///
    /// # Errors
    ///
    /// Returns the first version, TTL, or codec-list invariant violation.
    pub fn validate(&self) -> Result<(), DescriptorError> {
        if self.resurrect_version != RESURRECT_VERSION {
            return Err(DescriptorError::UnsupportedVersion(self.resurrect_version));
        }
...
pub enum DescriptorError {
    ...
    #[error("unsupported Resurrect version {0}; expected 1")]
