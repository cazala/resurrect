...
impl Namespace {
    ...
    ///
    /// This is exactly `keccak256("resurrect:" + application + ":" + major)`.
    #[must_use]
    pub fn derive(application_identifier: &str, major_protocol_version: u64) -> Self {
        let input = format!("resurrect:{application_identifier}:{major_protocol_version}");
...
mod tests {
    ...
    fn serde_round_trip_uses_canonical_hex() {
        let namespace = Namespace::derive("resurrect-test", 1);
