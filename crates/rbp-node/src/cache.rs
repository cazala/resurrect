    ...
    async fn load(&self, namespace: Namespace) -> Result<Vec<PeerCandidate>, CacheError> {
        ...
        for row in rows {
            ...
            let policy = AnnouncementPolicy {
                ...
                chain_time: now,
                max_record_bytes: rbp_core::DEFAULT_MAX_RECORD_BYTES,
                max_endpoints: rbp_core::DEFAULT_MAX_ENDPOINTS_PER_RECORD,
...
mod tests {
    use super::*;
    use rbp_core::{Endpoint, RECORD_TYPE_LIBP2P};
    use rbp_libp2p::{EndpointPolicy, Keypair, Libp2pPeerRecordCodec, Multiaddr, sign_peer_record};
