use crate::{
    BlockInfo, BlockReference, ProviderError, RBPRegistryV1, RegistryConstants, RegistryProvider,
};
use alloy::{
...
impl RegistryProvider for AlloyRegistryProvider {
    ...
    async fn registry_constants(
        ...
        let contract = RBPRegistryV1::new(address, &self.provider);
    ...
    async fn announcements(
        ...
        let filter = Filter::new()
            .address(address)
            .event_signature(RBPRegistryV1::PeerAnnounced::SIGNATURE_HASH)
        ...
        logs.into_iter()
            ...
            .map(|log| {
                ...
                let decoded = log
                    .log_decode_validate::<RBPRegistryV1::PeerAnnounced>()
