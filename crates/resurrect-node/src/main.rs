//! Native command-line process for the Resurrect v1 reference node.
...
use resurrect_libp2p::{EndpointPolicy, EnrCodec, Libp2pPeerRecordCodec};
use resurrect_node::{
...
struct Cli {
    ...
    #[arg(long, default_value = "resurrect-identity.key")]
    identity: PathBuf,

    /// Disposable verified-peer `SQLite` cache.
    #[arg(long, default_value = "resurrect-peers.sqlite3")]
    cache: PathBuf,
    ...
    #[arg(long, env = "RESURRECT_ETHEREUM_PRIVATE_KEY", hide_env_values = true)]
...
async fn run(cli: Cli) -> Result<()> {
    ...
    let provider: Arc<dyn RegistryProvider> = match cli.ethereum_private_key.as_deref() {
        Some(secret) => {
            let signer = PrivateKeySigner::from_str(secret)
                .context("Resurrect Ethereum signing key is invalid")?;
    ...
    if cli.seed && cli.ethereum_private_key.is_none() {
        bail!("--seed requires RESURRECT_ETHEREUM_PRIVATE_KEY");
    }
    ...
    let cache = SqlitePeerCache::open(
        ...
        resurrect_core::DEFAULT_MAX_ACTIVE_CANDIDATES,
    )
    .await?;
    ...
    for record_type in &descriptor.accepted_record_types {
        match *record_type {
            ...
            RECORD_TYPE_LIBP2P => codecs.register(Arc::new(Libp2pPeerRecordCodec::new(
                ...
                resurrect_core::DEFAULT_MAX_ENDPOINTS_PER_RECORD,
            ))),
