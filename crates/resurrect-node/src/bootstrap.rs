...
/// Native discovery adapter that can retain verified Resurrect records.
#[async_trait]
...
pub enum BootstrapState {
    ...
    AnnounceSelf,
    /// Waiting before the next native/Resurrect retry.
    Backoff,
...
pub struct BootstrapPolicy {
    ...
    /// Maximum simultaneous Resurrect/cache/native dials.
...
/// Interface-driven Resurrect startup controller.
#[derive(Debug)]
    ...
    A: AnnouncementPublisher + ?Sized,
{
    ...
    /// Executes cache → native → Resurrect → isolated promotion exactly once.
    ///
    ...
    pub async fn run_cycle(
        ...
        transitions.push(BootstrapState::ResurrectScan);
        if let Ok(peers) = self.registry.discover(namespace).await {
            for peer in &peers {
                let _ = self.native.add_verified_peer(peer.clone()).await;
            }
            if self
                .dial_candidates(peers, &mut transitions, BootstrapState::ResurrectScan)
                .await
            {
                return Ok(connected_outcome(
                    transitions,
                    BootstrapState::ResurrectScan,
                ));
            }
        }
...
mod tests {
    use super::*;
    use resurrect_core::{DiscoverySourceKind, Endpoint};
    use std::sync::{
        Arc, Mutex,
    ...
    async fn follows_cache_native_registry_order_and_stops_connected() {
        ...
        assert_eq!(outcome.connected_via, Some(BootstrapState::ResurrectScan));
