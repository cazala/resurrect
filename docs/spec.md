# Rebootable Bootstrap Protocol (RBP) v1
...
The Rebootable Bootstrap Protocol (RBP) is a generic bootstrap layer for peer-to-peer networks that must remain recoverable even if every application-specific node, bootstrap server, domain, and original operator disappears.
...
1. Each P2P application defines an RBP **network descriptor** containing an Ethereum registry address and a namespace.
...
5. When native discovery fails, nodes query recent RBP announcements.
6. If no live peer can be found, an eligible node announces itself and becomes the first seed of the rebooted network.
7. Once connectivity is established, ordinary P2P discovery takes over and RBP leaves the hot path.
...
RBP deliberately treats the Ethereum registry as **untrusted discovery information**. An onchain announcement is not an endorsement of a peer.
...
- Partition healing when isolated network components later query RBP.
...
RBP v1 does NOT attempt to provide:
...
An application-specific P2P overlay that uses RBP only for bootstrap/recovery.
...
Any discovery mechanism used by the application outside RBP: discv5, TopDisc, Kademlia DHT, libp2p peer exchange, configured peers, mDNS, DNS discovery, etc.

### 5.7 Reboot

The process by which a network that currently has no reachable peers forms a connected component again using RBP.
...
RBP announcements MUST be treated as attacker-controlled input until the embedded peer record has been cryptographically validated.
...
A peer record from years ago is unlikely to identify a reachable peer. RBP therefore bounds TTL and stores announcements only in logs.
...
Every RBP-enabled application MUST define or embed an RBP Network Descriptor.
...
| Field | Type | Meaning |
|---|---|---|
| `rbpVersion` | integer | MUST be `1` for this specification. |
| `registry.chainId` | uint256 | EVM chain containing the RBP registry. |
...
The descriptor is application configuration. RBP does not specify how a participant learns its application's descriptor.
...
```text
keccak256("rbp:example-network:1")
```
...
The RBP Network Descriptor intentionally does **not** contain a required JSON-RPC hostname. The descriptor identifies the registry chain and contract; the client obtains an RPC/provider independently.

An RBP implementation MUST support a caller-supplied registry-chain transport/provider abstraction.

A browser or static/immutable RBP client SHOULD support both of the following access modes:
...
A client using an injected provider for RBP discovery MUST NOT request account exposure merely to read registry state. In particular, discovery does not require `eth_requestAccounts`. Read-only calls such as `eth_chainId`, `eth_getBlockByNumber`, and `eth_getLogs` are sufficient.
...
- bundled providers are convenience infrastructure, not part of RBP consensus or identity.
...
RBP separates the Ethereum announcement envelope from the peer identity/address format.
...
Future RBP revisions may assign additional codec IDs.
...
RBP does not alter ENR signing or node-ID derivation.
...
An application using RBP MUST specify at least one accepted record type and MUST define how a validated peer record is converted into a dial attempt.

A node MUST NOT attempt to interpret an unknown record type.

### 9.4 Dial contexts and browser reachability

RBP peer records carry signed endpoint information, but RBP does not define which transport an application must use. An application profile MUST define how endpoints are selected for each client environment it intends to support.
...
These names are descriptive only and are not encoded by the RBP registry.

If an application requires an immutable browser client to bootstrap from RBP, its application profile MUST identify at least one browser-dialable endpoint representation that can be carried inside an accepted signed peer-record codec. Examples include browser-capable libp2p WebTransport/WebSocket multiaddrs or an HTTPS/libp2p-HTTP multiaddr.
...
RBP itself defines no `HTTP`, `SOLVER`, `RELAY`, or other application capability bitset. Such capabilities belong to the application protocol or its peer handshake.
...
A reorg MUST NOT create application correctness issues because RBP announcements are merely hints. If an event disappears in a reorg, the peer may remain in the local peer cache if independently reachable.
...
RBP defines behavioral roles only; there is no onchain role registry.

### 12.1 Light node

- Uses RBP to find peers.
...
- MAY publish an RBP announcement when publicly reachable.

### 12.3 Seed

- Intentionally acts as a bootstrap target.
- SHOULD have stable, publicly reachable endpoints.
- SHOULD maintain a non-expired RBP announcement.
...
async function bootstrap(descriptor):
    ...
    announcements = await scanRbpRegistry(descriptor)
    ...
    if isEligibleSeed():
        await publishRbpAnnouncement(mySignedPeerRecord(), REBOOT_TTL)

    while not shutdown:
        peers = await nativeDiscovery(timeout=RETRY_INTERVAL)
        if not peers:
            peers = await scanRbpRegistry(descriptor)
...
maximum simultaneous RBP dials: 8
```
...
2. has failed local/native/RBP discovery for a configured interval, and
...
RBP SHOULD be used as a fallback alongside existing P2P discovery.

### 14.1 discv5

EIP-778 ENRs are directly compatible with Ethereum Node Discovery v5.

Modern discv5 specifications also define topic-based service discovery (TopDisc) for discovering application/service participants on the shared discovery substrate. Applications MAY use TopDisc as the normal discovery path and keep RBP as the cold-start fallback.

Reference: <https://github.com/ethereum/devp2p/blob/master/discv5/discv5.md>

### 14.2 libp2p

libp2p applications SHOULD feed validated RBP records into their normal peer store and then rely on their normal discovery stack, such as:
...
RBP does not replace these mechanisms.
...
RBP itself does not decide whether addresses are useful. Implementations MUST apply local policy.
...
RBP's baseline Sybil cost is the cost of onchain publication. This is not sufficient for all threat models.
...
Applications with stronger adversarial requirements MAY add their own authenticated membership, staking, proof-of-work, reputation, or capability checks **after** RBP discovery. Such mechanisms are outside RBP.
...
RBP cannot guarantee eclipse resistance if the only reachable peers are attacker-controlled.

Applications SHOULD preserve and use independent native discovery mechanisms. RBP is intended as one source of signed candidate endpoints, not as an authoritative membership list.
...
If the registry chain is unavailable or censoring announcements, RBP cannot create a new root rendezvous. Existing connected networks can continue operating using their native P2P protocols.

This is an explicit RBP trust/liveness assumption.
...
A browser client using an EIP-1193 provider SHOULD perform read-only RBP discovery before requesting account access, so peer discovery does not unnecessarily reveal wallet addresses to the application.
...
A reference RBP implementation MAY expose a local/admin HTTP API. This API is not part of the network protocol and MUST NOT be required for interoperability.
...
The reference implementation SHOULD NOT make Node.js a requirement for operating a full RBP seed. A browser/TypeScript client is valuable, but the long-lived reference seed should be a native process with predictable networking behavior and resource limits.

For new Rust code, Alloy is RECOMMENDED over legacy `ethers-rs` APIs. RBP itself does not require EIP-7702, but using Alloy keeps the Ethereum integration current and gives dependent protocols one modern provider/type stack.

### 21.2 Repository layout

A recommended standalone RBP repository layout is:
...
rbp/
  ...
  crates/
    ...
    rbp-discv5/              # optional / later phase
      src/
        enr.rs
        discovery.rs
        topdisc.rs
...
RBP is intended to be consumed by unrelated application protocols as released libraries/packages. Dependent projects SHOULD depend on released `rbp-*` crates/packages rather than copying the source or using a git submodule that makes the two projects operationally inseparable.
...
libp2p known peers / peer exchange
    ↓
RBP Ethereum registry
```

Support for ENR/discv5/TopDisc SHOULD be implemented behind the discovery abstraction and MAY ship after the minimal reboot behavior is proven. The RBP correctness and resurrection properties MUST NOT depend on TopDisc or any other still-evolving discovery extension.
...
- attempts to bootstrap through RBP after native discovery failure,
- can operate normally without continuing to use RBP once connected.
...
A browser/static implementation claiming rebootable RBP discovery conformance SHOULD additionally:
...
2. Start node A.
3. A fails native/RBP discovery.
...
- injected EIP-1193 provider can scan RBP without `eth_requestAccounts`.
...
- Rust `rbp-core`, `rbp-ethereum`, and `rbp-node` crates,
- Tokio runtime,
...
The production goal is that healthy nodes rarely need RBP after first bootstrap.
...
| All configured bootstrap peers die | RBP scan discovers other announced seeds. |
| DNS/domain disappears | RBP unaffected. |
...
| All peers and all copies of application data disappear | Connectivity can reboot; lost application data cannot be reconstructed by RBP. |
...
- [ ] Deploy an immutable RBP registry to a local EVM network.
...
- [ ] Start node B with no knowledge of A and have B discover A only through RBP.
